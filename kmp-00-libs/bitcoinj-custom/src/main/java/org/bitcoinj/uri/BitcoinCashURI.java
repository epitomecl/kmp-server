...
public class BitcoinCashURI {
    ...
    public BitcoinCashURI(String uri) throws BitcoinURIParseException {
        this(null, uri);
    }

    public BitcoinCashURI(@Nullable NetworkParameters params, String input) throws BitcoinURIParseException {
        String schemeSpecificPart;
        URI uri;
        Preconditions.checkNotNull((Object)input);
        String scheme = null == params ? "bitcoincash" : params.getUriScheme();
        try {
            uri = new URI(input);
        }
        ...
        if (addressSplitTokens.length == 0) {
            throw new BitcoinURIParseException("No data found after the bitcoincash: prefix");
        }
    ...
    private void parseParameters(@Nullable NetworkParameters params, String addressToken, String[] nameValuePairTokens) throws BitcoinURIParseException {
        for (String nameValuePairToken : nameValuePairTokens) {
            ...
            if (sepIndex == -1) {
                throw new BitcoinURIParseException("Malformed Bitcoin Cash URI - no separator in '" + nameValuePairToken + "'");
            }
            if (sepIndex == 0) {
                throw new BitcoinURIParseException("Malformed Bitcoin Cash URI - empty name '" + nameValuePairToken + "'");
            }
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("BitcoinCashURI[");
        boolean first = true;
    ...
    public static String convertToBitcoinCashURI(Address address, Coin amount, String label, String message) {
        return BitcoinCashURI.convertToBitcoinCashURI(address.getParameters(), address.toString(), amount, label, message);
    }

    public static String convertToBitcoinCashURI(NetworkParameters params, String address, @Nullable Coin amount, @Nullable String label, @Nullable String message) {
        ...
        if (label != null && !"".equals(label)) {
            ...
            builder.append(FIELD_LABEL).append("=").append(BitcoinCashURI.encodeURLString(label));
        }
        if (message != null && !"".equals(message)) {
            ...
            builder.append(FIELD_MESSAGE).append("=").append(BitcoinCashURI.encodeURLString(message));
        }
        return builder.toString();
    }
